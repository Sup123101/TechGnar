...
namespace VRWeapons
{
    public class Bolt : MonoBehaviour, IBoltActions
    {
        ...
        [Tooltip("Location of round on bolt face. Align round with desired location, then press \"Assign Round Snap Location\"."), SerializeField]
        ...
        public IBulletBehavior ChamberNewRound()
        {
            if (thisWeap.Magazine != null)
            {
                ...
                {
                    Collider col = chamberedRoundT.GetComponent<Collider>();
                    if (col != null)
                    {
